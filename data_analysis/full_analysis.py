...
def find_slope_R_difference(peak, peak_err, split_indices=split_indices, m_peaks=m_peaks, num_files=num_files):
    ...
    plt.plot(m, average_slope*m+average_intercept, label ='Average Slope: $R^2$ = ({:.2e})m - {:.2e}'.format(average_slope,-average_intercept), linestyle = 'dashed', color = 'k')
    plt.plot(m, (average_slope+slope_err)*m+(average_intercept), label = 'Error Bounds', linestyle = 'dashed', color = 'b')
    plt.plot(m, (average_slope-slope_err)*m+(average_intercept), linestyle = 'dashed', color = 'b')
    plt.xlabel('m'); plt.ylabel('$R^2$')
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', frameon=False)
    plt.annotate("$\u03c7_\u03bd^2$ = {:.2f}".format(red_chi_sq(m_obs, [average_slope*m+average_intercept for m in m])), (0, 1))
...
def plot_BvsI (B_err, I_err):
    ...
    plt.plot(I_tot, a*np.array(I_tot) + b, 'k-', label='B = ({:.2e})I + {:.2e} G'.format(a,b))
    plt.annotate("$\u03c7_\u03bd^2$ = {:.2f}".format(red_chi_sq(B_tot, a*np.array(I_tot) + b)),(1, 0))
...
plt.plot(B, dL_exp, 'r', label='\u0394 \u03BB = ({:.2e})B - {:.2e} m'.format(a,-b))
...
plt.plot(B, dE_exp, 'r', label='$\u0394$E = ({:.2e})B - {:.2e} J'.format(a,-b))
